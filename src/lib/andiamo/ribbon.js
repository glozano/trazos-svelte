...
import {StrokeQuad} from '$lib/andiamo/stroke';
import {MIN_POS_CHANGE, RIBBON_DETAIL, SMOOTH_COEFF, NORM_FACTOR, MIN_CTRL_CHANGE, TEXCOORDU_INC} from '$lib/andiamo/parameters';
import {BSpline} from '$lib/andiamo/splines';
...
class Ribbon {
  ...
  addPoint = function(gesture, time, col, alp, x, y) {
    this.pX = x;
    this.pY = y;
    ...
    if (this.nControl == 4) {
      ...
      var p1 = this.p5.createVector(this.pX, this.pY, 0);
      var p0 = this.p5.createVector(this.pX0, this.pY0, 0);
      var p10 = this.p5.constructor.Vector.sub(p0, p1);
      var p_1 = this.p5.constructor.Vector.add(p0, p10);
      var p_2 = this.p5.constructor.Vector.add(p_1, p10);
    ...
    var dir = this.p5.createVector(dX, dY, dZ);
    var nor = this.p5.createVector(nX, nY, nZ);
    this.oldVel = this.newVel;
    var l = dir.mag();
    this.newVel = this.ribbonsWidth / this.p5.map(l, 0, 100, 1, NORM_FACTOR + 0.1);
  ...
  addControlPoint = function(spline, newX, newY, newZ, nor, vel) {
    var addCP = true;
    var cp1 = this.p5.createVector(newX - vel * nor.x, newY - vel * nor.y, newZ - vel * nor.z);
    if (1 < this.nControl) {
      var cp0 = this.p5.createVector(0, 0);
  ...
  drawRibbonStretch = function(gesture, time, col, alp, spline1, spline2) {
    var ti;
    var t;
    var x, y, z;

    if (!this.Sid1Point0) this.Sid1Point0 = this.p5.createVector(0, 0);
    if (!this.Sid1Point1) this.Sid1Point1 = this.p5.createVector(0, 0);
    if (!this.Sid2Point0) this.Sid2Point0 = this.p5.createVector(0, 0);
    if (!this.Sid2Point1) this.Sid2Point1 = this.p5.createVector(0, 0);
    ...
    for (ti = 1; ti <= 10; ti++) {
      if (ti % this.ribbonDetail == 0) {
        ...
        var quad = new StrokeQuad(this.p5,time);
        var r = col[0];
        var g = col[1];
        var b = col[2];
        var a = alp;
